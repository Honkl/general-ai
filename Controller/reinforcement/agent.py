...
class Agent():
    ...
    def play(self, env_state, epoch_number):
        # Probability of random action selection will decay in time
        if np.random.random() < self.random_action_prob ** epoch_number:
            estimated_reward = self.session.run(self.estimated_reward, {self.state: [env_state]})
